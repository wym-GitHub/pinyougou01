...
public class TypeTemplateServiceImpl implements TypeTemplateService {
    ...
    public PageResult findPage(TbTypeTemplate typeTemplate, int pageNum, int pageSize) {
        ...
        saveToRedis();//缓存所有模板中的规格,和品牌,放到缓存中,以供前台读取,不能放在分页和page中间
        return new PageResult(page.getTotal(), page.getResult());
    }

    @Override
    public List<Map> typeOptions() {
        List<Map> maps = typeTemplateMapper.typeOptions();
        return maps;
    }
    ...
    public void saveToRedis() {
        List<TbTypeTemplate> all = findAll();//获取所有模板数据
        for (TbTypeTemplate tbTypeTemplate : all) {
            List<Map> maps = JSON.parseArray(tbTypeTemplate.getBrandIds(), Map.class);//获取模板中的品牌集合
            redisTemplate.boundHashOps("brandList").put(tbTypeTemplate.getId(), maps);

            List<Map> specList = findSpecList(tbTypeTemplate.getId());//获取规格列表
